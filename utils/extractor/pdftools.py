...
from utils.extractor.pdf_process import process_single_pdf_text,extract_methods_and_discussion_from_txt
import re
import numpy as np
import hashlib
...
class PDF:
    def __init__(self, file, safe_doi):
        ...
        self._text =  process_single_pdf_text(file)

    def get_images(self, use_scaled):
        folder = 'temp/images/' + self.safe_doi
        os.mkdir(folder)
        
        image_list = subprocess.Popen(('pdfimages', '-list', self.file), stdout=subprocess.PIPE)
        n_images = subprocess.check_output(('wc', '-l'), stdin=image_list.stdout)
        image_list.wait()
        n_images = int(n_images.decode("utf-8"))
    ...
    def get_text(self, section):
        ...
        try:
            ...
            if section == 'methods':
                ...
                #         vals[i + 2] = 1
                # text = ' '.join([sents[i] for i in range(len(vals)) if vals[i]])
                return extract_methods_and_discussion_from_txt(text,section='methods')
            elif section == 'discussion':
                return extract_methods_and_discussion_from_txt(text,section='discussion')
            elif section == 'all':
                return extract_methods_and_discussion_from_txt(text,section='all')
